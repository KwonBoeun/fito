...
function loadData() {
  ...
  gid = parseInt(window.GROUP_ID);
  group = store.groups.find(g => g.id === gid);
  if (!group) {
    const rec = (store.recommendGroups || []).find(g => g.id === gid);
    if (rec) group = { ...rec, joined:false, myRole:'none', liveCnt:0, chatCnt:0, hasLive:false, liveViewers:0, inactive:false };
  }
  if (!group) { showSnackbar('해당 그룹이 존재하지 않습니다.'); return false; }
  myRole = group.myRole || 'none';
  members = (store.groupMembers && store.groupMembers[gid]) || [];
  lives = (store.groupLives && store.groupLives[gid]) || [];
  stories = (store.groupStories && store.groupStories[gid]) || [];
  return true;
}
...
function renderGroupInfo() {
  ...
  document.getElementById('groupTags').innerHTML = (group.tags||[]).map(t => `<span class="gm-tag">${t}</span>`).join('');

  // 가입/탈퇴 버튼
  const btn = document.getElementById('joinLeaveBtn');
  if (group.joined) { btn.textContent='탈퇴'; btn.className='gm-join-btn leave'; }
  else { btn.textContent='가입 신청'; btn.className='gm-join-btn join'; }
  ...
  if (!group.joined) {
    ...
    document.getElementById('newLiveWrap').style.display = 'none';
    document.getElementById('storageSection').style.display = 'none';
  }
}
...
function renderIssues() {
  ...
  if (stories.length === 0) {
    const msg = group.joined
      ? '업로드 된 핏츠가 없습니다. 첫 번째 핏츠의 주인공이 되어 보세요!'
      : '업로드 된 핏츠가 없습니다.';
    scroll.innerHTML = `<span style="font-size:11px;color:var(--gray-400);padding:8px 0">${msg}</span>`;
  } else {
    scroll.innerHTML = stories.map(s =>
      `<div class="gm-story-item" title="${s.author}: ${s.title}"></div>`
    ).join('');
  }

  // 오늘의 Chat (스토어의 getTodayBestChat 사용)
  const chatText = document.getElementById('todayChatText');
  const chatAuthor = document.getElementById('todayChatAuthor');
  const bestChat = FITO_STORE.getTodayBestChat(gid);

  if (bestChat) {
    chatText.textContent = bestChat.text;
    chatAuthor.textContent = `— ${bestChat.author} ♥ ${bestChat.likes}`;
  } else {
    // 날짜가 안 맞으면 Mock 데이터에서 가장 좋아요 많은 것으로 대체 표시
    const allChatId = ((store.groupChats||{})[gid]||[]).find(c => c.type==='all');
    if (allChatId) {
      const msgs = (store.groupMessages||{})[allChatId.id] || [];
      ...
      if (qualified.length > 0) {
        ...
        chatText.textContent = qualified[0].text;
        chatAuthor.textContent = `— ${qualified[0].author} ♥ ${qualified[0].likes}`;
      } else {
...
function openStories() {
  if (stories.length === 0) { showSnackbar('핏츠가 없습니다.'); return; }
  renderStoryList();
...
function renderStoryList() {
  let sorted = [...stories];
  if (storySortType === 'popular') sorted.sort((a,b) => (b.views||0) - (a.views||0));
  document.getElementById('storyList').innerHTML = sorted.map(s => `
    <div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--gray-100)">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:11px;color:var(--gray-400)">${s.author} · ${s.time} · 조회 ${s.views||0}회</div>
      </div>
    </div>
  `).join('');
}
...
function renderChats() {
  ...
  list.innerHTML = chats.map(c => {
    ...
    const unread = Math.floor(Math.random() * 5); // mock
    const icon = c.type==='all'
      ? '<svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>'
      : '<svg viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>';
    const badge = unread > 0 ? `<span class="gm-chat-badge">${unread}</span>` : '';

    // 비밀채팅 접근 권한 체크
    const hasAccess = c.type === 'all' || (c.allowedMembers && c.allowedMembers.includes(myName));
    const lockStyle = hasAccess ? '' : 'opacity:0.4';

    return `<div class="gm-chat-item" style="${lockStyle}" onclick="openChat('${c.id}',${hasAccess})">
      <div class="gm-chat-icon">${icon}</div>
      <div class="gm-chat-info">
        <div class="gm-chat-name">${c.name}${!hasAccess?' 🔒':''}</div>
        <div class="gm-chat-preview">${hasAccess ? lastMsg : '접근 권한이 없습니다'}</div>
      </div>
      ${hasAccess ? badge : ''}
    </div>`;
  }).join('');
...
function renderLives() {
  ...
  if (lives.length === 0) {
    list.innerHTML = '<div class="gp-empty">라이브가 만들어지지 않았습니다.<br>새로운 라이브를 만들어 보는건 어떨까요?</div>';
    return;
  }
  const order = {live:0,soon:1,scheduled:2};
  const sorted = [...lives].sort((a,b)=>(order[a.status]||2)-(order[b.status]||2));
  list.innerHTML = sorted.map(l => {
    let statusHtml='', metaText='';
    if (l.status==='live') { statusHtml='<span class="gm-live-status live">● LIVE</span>'; metaText=`${l.participants}/${l.max}명 참여 중`; }
    else if (l.status==='soon') { statusHtml='<span class="gm-live-status soon">곧 시작</span>'; metaText=`${l.startTime} 시작 예정`; }
    else { statusHtml='<span class="gm-live-status scheduled">예정</span>'; metaText=`${l.startTime} 시작 예정`; }
    return `<div class="gm-live-card" onclick="joinLive(${l.id})">
      ...
      <div class="gm-live-info"><div class="gm-live-title">${l.title}</div><div class="gm-live-meta">${l.host} · ${metaText}</div>${statusHtml}</div>
    </div>`;
  }).join('');
}
...
function toggleMembership() {
  if (group.joined) document.getElementById('leaveModal').classList.add('open');
  else document.getElementById('joinModal').classList.add('open');
}
...
function submitJoin() {
  const g = document.getElementById('joinGreeting').value.trim();
  if (g.length < 1 || g.length > 100) { showSnackbar('가입인사를 1~100자로 입력해주세요.'); return; }
  closeJoinModal(); showSnackbar('가입 신청이 전송되었습니다.');
}
function confirmLeave() {
  FITO_STORE.update(s => {
    const g = s.groups.find(g => g.id === gid);
    if (g) { g.joined = false; g.myRole = 'none'; }
    if (s.groupMembers[gid]) s.groupMembers[gid] = s.groupMembers[gid].filter(m => m.name !== (s.myName||'나'));
  });
  closeLeaveModal(); showSnackbar('그룹에서 탈퇴했습니다.');
...
function renderMembersList() {
  ...
  if (isAdmin && pending.length > 0) {
    ...
    html += pending.map((p,i) => `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--gray-100)">
      ...
      <button onclick="acceptPending(${i})" style="padding:4px 10px;background:#000;color:#fff;border:none;border-radius:999px;font-size:11px;font-weight:600;cursor:pointer">수락</button>
      <button onclick="rejectPending(${i})" style="padding:4px 10px;background:#f2f2f2;color:#e05c4b;border:none;border-radius:999px;font-size:11px;font-weight:600;cursor:pointer">거절</button>
    </div>`).join('');
    html += '<div style="height:12px"></div>';
  }
  html += '<div style="font-size:12px;font-weight:700;color:var(--gray-500);margin-bottom:8px">멤버</div>';
  const rl={owner:'그룹장',manager:'매니저',member:'그룹원'};
  const rc={owner:'var(--orange)',manager:'var(--blue)',member:'var(--gray-400)'};
  html += members.map(m => {
    let act='';
    if (myRole==='owner'&&m.role==='member'&&m.name!==(store.myName||'나')) {
      act=`<button onclick="promoteMember('${m.name}')" style="padding:3px 8px;background:var(--blue);color:#fff;border:none;border-radius:999px;font-size:10px;cursor:pointer;margin-right:4px">승급</button><button onclick="kickMember('${m.name}')" style="padding:3px 8px;background:#fdecea;color:#e05c4b;border:none;border-radius:999px;font-size:10px;cursor:pointer">내보내기</button>`;
    } else if (myRole==='manager'&&m.role==='member'&&m.name!==(store.myName||'나')) {
      act=`<button onclick="showSnackbar('승급 요청을 그룹장에게 전달했습니다.')" style="padding:3px 8px;border:1px solid #e5e5e5;background:#fff;border-radius:999px;font-size:10px;cursor:pointer">승급 요청</button>`;
    }
    return `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--gray-100)">
      ...
      <span style="font-size:10px;font-weight:700;color:${rc[m.role]};padding:2px 8px;background:var(--gray-50);border-radius:999px">${rl[m.role]}</span>${act}
    </div>`;
  }).join('');
  list.innerHTML = html;
}
...
function openChat(chatId, hasAccess) {
  if (!group.joined) { showSnackbar('그룹에 가입해야 채팅에 참여할 수 있습니다.'); return; }
  if (!hasAccess) { showSnackbar('이 비밀 채팅방에 접근할 권한이 없습니다.'); return; }
  location.href = `${window.URL_GROUP_CHAT}&type=${chatId}`;
}
function addChat() {
...
function createNewLive() {
  ...
  liveCreateLock = true; setTimeout(()=>{liveCreateLock=false;}, 2000);
...
document.addEventListener('DOMContentLoaded', () => {
  if (!loadData()) return;
  renderGroupInfo(); renderIssues(); renderChats(); renderLives();
});
