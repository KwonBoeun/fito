...
/* ── 유저 업로드 커뮤니티/질문 병합 (DATA 선언 전 실행 → 나중에 덮어씀) ── */
/* 실제 병합은 COM_DATA, Q_DATA 선언 직후에 수행 */
...
/* ── 유저 업로드 커뮤니티 병합 ── */
(function mergeUserCommunity() {
  try {
    const raw = localStorage.getItem('fito_user_community');
    if (!raw) return;
  ...
  } catch(e) { console.warn('mergeUserCommunity error', e); }
})();

/* ── 유저 업로드 질문 병합 ── */
(function mergeUserQuestion() {
  try {
    const raw = localStorage.getItem('fito_user_question');
    if (!raw) return;
    const items = JSON.parse(raw);
    if (!Array.isArray(items) || !items.length) return;
